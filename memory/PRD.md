...
- [x] **Device-aware asset highlighting** — auto-detects user's platform from browser UA, scores every asset, marks the best match with a "For your device" badge. Manual picker (Auto / iOS / macOS Apple / macOS Intel / Linux x64 / Linux ARM / Windows / All) lets users override.
- [x] **Changelog diff view** at `/api/tools/{id}/releases/changelog?from_tag=...` — when a user has previously downloaded a binary (tracked in `localStorage`), shows a cyan "Update available" panel listing every release between their version and the latest, with expandable release notes per entry.
